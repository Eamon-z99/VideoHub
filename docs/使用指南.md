# VideoHub 使用指南

> 两套方案：Python批量转码（测试）+ Spring Boot上传转码（生产）

---

## 📋 方案概述

### 方案一：Python批量转码（测试用）
- **用途**：一次性处理1000个MP4视频
- **脚本**：`scripts/batch_transcode.py`
- **输出**：`E:\VideoHubStorage\videos\{video_id}\`

### 方案二：Spring Boot上传转码（生产用）
- **用途**：用户通过前端上传视频，后端自动转码
- **接口**：`POST /api/upload`
- **输出**：相同的存储结构

**两套方案使用完全相同的存储结构，确保兼容！**

---

## 🚀 方案一：Python批量转码（立即开始）

### Step 1：准备环境

1. **安装FFmpeg**
   - 下载：https://ffmpeg.org/download.html
   - 解压后添加到PATH，或记录完整路径

2. **创建存储目录**
   ```
   E:\VideoHubStorage\
     ├─ videos\          # 转码后的视频（Python脚本输出）
     ├─ source\          # 原始MP4备份（可选）
     └─ temp\            # 临时文件
   ```

3. **准备Python环境**
   ```bash
   python --version  # 确保Python 3.7+
   ```

### Step 2：配置脚本

打开 `scripts/batch_transcode.py`，修改配置：

```python
# 第12-13行
SOURCE_DIR = r"E:\YourMp4Videos"  # ⚠️ 改成你的1000个MP4目录
OUTPUT_ROOT = r"E:\VideoHubStorage\videos"  # 输出目录

# 第16-17行（如果FFmpeg不在PATH中）
FFMPEG_PATH = r"C:\ffmpeg\bin\ffmpeg.exe"  # 完整路径
FFPROBE_PATH = r"C:\ffmpeg\bin\ffprobe.exe"
```

### Step 3：测试单个视频

1. **临时修改脚本**（第280行左右）：
   ```python
   mp4_files = [mp4_files[0]]  # 只处理第一个
   ```

2. **运行脚本**：
   ```bash
   cd D:\vue_project\VideoHub
   python scripts/batch_transcode.py
   ```

3. **验证输出**：
   ```
   E:\VideoHubStorage\videos\video_0001\
     ├─ manifest.json
     ├─ videoInfo.json
     ├─ video_1080p\
     │   ├─ init.m4s
     │   ├─ seg-001.m4s
     │   └─ ...
     ├─ video_720p\
     ├─ video_480p\
     └─ audio_128k\
   ```

### Step 4：全量转码

1. **恢复脚本**（删除测试限制）

2. **运行批量转码**：
   ```bash
   python scripts/batch_transcode.py
   ```

3. **预计时间**：
   - 每个视频约5-10分钟
   - 1000个视频需要几天时间
   - 建议分批处理或后台运行

---

## 🎯 方案二：Spring Boot上传转码（生产环境）

### Step 1：配置后端

1. **检查 `application.properties`**
   ```properties
   video.storage.root=E:\\VideoHubStorage
   video.storage.videos=${video.storage.root}\\videos
   video.storage.source=${video.storage.root}\\source
   video.storage.temp=${video.storage.root}\\temp
   
   ffmpeg.path=ffmpeg  # 如果在PATH中，否则用完整路径
   ffprobe.path=ffprobe
   ```

2. **确保FFmpeg可用**
   - 后端启动时会调用FFmpeg
   - 确保FFmpeg在PATH中，或修改配置使用完整路径

### Step 2：启动后端

```bash
cd D:\vue_project\VideoHub\backend\VideoBackend
mvn spring-boot:run
```

### Step 3：测试上传接口

**使用Postman或curl测试：**

```bash
curl -X POST http://localhost:8081/api/upload \
  -F "file=@your_video.mp4" \
  -F "title=测试视频"
```

**响应示例：**
```json
{
  "videoId": "video_abc12345",
  "message": "上传成功，正在转码中...",
  "status": "PROCESSING",
  "progress": 0
}
```

### Step 4：查询转码状态

```bash
curl http://localhost:8081/api/video/video_abc12345/status
```

**响应示例：**
```json
{
  "videoId": "video_abc12345",
  "status": "DONE",
  "progress": 100,
  "message": "转码完成"
}
```

### Step 5：前端集成

**上传组件示例：**

```vue
<template>
  <el-upload
    :action="uploadUrl"
    :on-success="handleSuccess"
    :before-upload="beforeUpload"
  >
    <el-button>上传视频</el-button>
  </el-upload>
</template>

<script setup>
import { ref } from 'vue'
import request from '@/utils/request'

const uploadUrl = '/api/upload'

const handleSuccess = (response) => {
  console.log('上传成功:', response.videoId)
  // 轮询查询转码状态
  checkStatus(response.videoId)
}

const checkStatus = async (videoId) => {
  const { data } = await request.get(`/video/${videoId}/status`)
  if (data.status === 'DONE') {
    // 转码完成，跳转到播放页
    router.push(`/video/${videoId}`)
  } else {
    // 继续轮询
    setTimeout(() => checkStatus(videoId), 3000)
  }
}
</script>
```

---

## 📊 存储结构（两套方案共用）

```
E:\VideoHubStorage\
├─ videos\                    # 所有转码后的视频
│  ├─ video_0001\            # Python脚本生成
│  │  ├─ manifest.json
│  │  ├─ videoInfo.json
│  │  ├─ video_1080p\
│  │  ├─ video_720p\
│  │  ├─ video_480p\
│  │  └─ audio_128k\
│  ├─ video_abc12345\        # Spring Boot上传生成
│  │  └─ ...（相同结构）
│  └─ ...
├─ source\                   # 原始MP4备份
│  ├─ video_0001.mp4
│  └─ ...
└─ temp\                     # 临时文件
```

---

## 🔍 API接口说明

### 1. 上传视频
```
POST /api/upload
Content-Type: multipart/form-data

参数：
- file: 视频文件（MP4）
- title: 视频标题（可选）

响应：
{
  "videoId": "video_xxx",
  "status": "PROCESSING",
  "progress": 0
}
```

### 2. 查询转码状态
```
GET /api/video/{videoId}/status

响应：
{
  "videoId": "video_xxx",
  "status": "DONE|PROCESSING|FAILED",
  "progress": 100
}
```

### 3. 获取播放清单
```
GET /api/playurl/{cid}

响应：
{
  "cid": "video_xxx",
  "duration": 300,
  "dash": {
    "video": [...],
    "audio": [...]
  }
}
```

### 4. 视频列表
```
GET /api/videos

响应：
[
  {
    "id": "video_xxx",
    "title": "视频标题",
    "duration": 300,
    ...
  }
]
```

---

## ✅ 验证步骤

### 1. Python脚本转码后验证

```bash
# 检查manifest.json是否存在
dir E:\VideoHubStorage\videos\video_0001\manifest.json

# 检查分片文件
dir E:\VideoHubStorage\videos\video_0001\video_1080p\*.m4s
```

### 2. 后端接口验证

```bash
# 测试playurl接口
curl http://localhost:8081/playurl/video_0001

# 测试视频列表
curl http://localhost:8081/api/videos
```

### 3. 前端播放验证

1. 访问：`http://localhost:5173/video/video_0001`
2. 应该能正常播放

---

## 🐛 常见问题

### 1. FFmpeg未找到

**错误**：`java.io.IOException: Cannot run program "ffmpeg"`

**解决**：
- 确保FFmpeg在PATH中
- 或修改 `application.properties`：
  ```properties
  ffmpeg.path=C:\\ffmpeg\\bin\\ffmpeg.exe
  ffprobe.path=C:\\ffmpeg\\bin\\ffprobe.exe
  ```

### 2. 转码失败

**检查**：
- FFmpeg版本是否支持DASH格式
- 源视频格式是否支持
- 磁盘空间是否充足

### 3. 静态资源404

**检查**：
- `application.properties` 中的路径是否正确
- Windows路径分隔符是否正确（`\\`）

---

## 📝 下一步

1. ✅ **Python批量转码**：处理1000个测试视频
2. ✅ **后端上传接口**：支持用户上传
3. 🔄 **前端上传页面**：添加上传UI
4. 🔄 **播放页多清晰度**：支持清晰度切换
5. 🔄 **视频列表页**：展示所有视频

---

## 💡 提示

- **两套方案完全兼容**：Python转码的视频和Spring Boot上传的视频使用相同结构
- **可以混合使用**：先用Python批量转码，后续用Spring Boot处理新上传
- **存储统一**：所有视频都在 `E:\VideoHubStorage\videos\` 下









